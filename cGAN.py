...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 23 15:39:02 2023
...
total_images = len(os.listdir(home+'RGB'))  # 360 images
...
# len(train_indices) = 288, len(val_indices)=72
...
class ColorisationDataset(Dataset):
    ''' Resizes the images to 256x256 pixel size and horizontally flips the images
        if in the training set as a method of data augmentation as the training
        set is not vast.
        This reads a grayscale image, converts it to RGB and then to Lab colour
...
# torch.Size([16,1,256,256]) 1-channel for greyscale, torch.Size([16,2,256,256]) 2-channels for a and b as expected
#print(len(trainloader), len(testloader))
# len(trainloader)=18, len(testloader)=5
...
# out.shape    # torch.Szie([16,1,30,30]) output shape 30x30 but doesn't mean patch size is 30x30 -> it's 70x70
# patch size is determined by computing receptive field of the 900 (30 times 30) output numbers which is 70 by 70
...
# create dictionary of loss results
...
# updates the losses after each epoch (in training)
...
# Take an image in l*a*b space and convert to rgb
def lab_to_rgb(L, ab):
...
#  Make a plot of the generator and discriminator loss
...
# visualise the generated images and actual images for comparison
...
# print results of the losses after each epoch        
...
def train_model(model, trainloader, epochs, display_every=30):
    ...
    start = time.time()
    data = iter(trainloader) # batch for visualising the model output after fixed intervals after training
...
# initialise the network and train the model
model = GANModel()
